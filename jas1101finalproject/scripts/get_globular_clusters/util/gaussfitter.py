...
downloaded from https://scipy-cookbook.readthedocs.io/items/FittingData.html
...
def gaussfit(
    ...
    """Gaussian fitter for 2D data.

    Can fit a variety of different forms of 2-dimensional Gaussians.
    ...
    def errorfunction(p):
        return ravel(
            (
                twodgaussian(p, circle, rotate, vheight)(*indices(data.shape))
                - data
            )
            / err
        )
