...
def covfnc1(x):
    ...
    return 0.0036 * numpy.exp(-x / 0.25) + 0.0008 * numpy.exp(-x / 20.0)


# /def
def covfnc2(x):
    """oscillatory choice (more optimistic)"""
    return 0.004 * numpy.sinc(x / 0.5 + 0.25) + 0.0008 * numpy.exp(-x / 20.0)
...
def angular_distance(ra0, dec0, ra1, dec1):
    ...
    return (
        ...
        * numpy.arcsin(
            (
                numpy.sin((dec0 - dec1) * 0.5 * d2r) ** 2
                + numpy.cos(dec0 * d2r)
                * numpy.cos(dec1 * d2r)
                * numpy.sin((ra0 - ra1) * 0.5 * d2r) ** 2
            )
            ** 0.5
        )
        / d2r
...
def get_mean_pm(
    ...
    """
    nstar = len(ra)
    pmboth = numpy.column_stack((pmra, pmdec)).reshape(
        -1
    )  # interleaved pmra,pmdec for each star
    ...
    sig2mul = (
        1
        / (1 + angular_distance(ra, dec, meanra, meandec) ** 2 / rsigma ** 2)
        ** 0.5
    )

    if (
        sigma is None and covfnc is None
    ):  # should have a separate (simplified) treatment for this case,
        covfnc = (
            lambda x: x * 0
        )  # but instead use the general machinery with covfnc set to zero
    ...
    if not covfnc is None:
        ...
        covmat = numpy.array(
            [
                covfnc(angular_distance(ra, dec, ra0, dec0))
        ...
        if not sigma is None:
            covmat2.flat[:: stride // 2] += numpy.repeat(
                sigma ** 2 * sig2mul, 2
            )
            ...
            covar = numpy.linalg.inv(
                numpy.dot(CinvP.T, P)
            )  # covariance of uncertainties in mean pmra,pmdec
            sol = numpy.dot(
                covar, numpy.dot(CinvP.T, pmboth)
            )  # mean pmra,pmdec (the solution)
            sigval = sigma  # known value provided as input
            sigerr = 0.0  # with no uncertainty

        else:

            Lambda, Q = scipy.linalg.eigh(
                covmat2, numpy.diag(numpy.repeat(sig2mul, 2))
            )
            ...
            def rootfindersigma(sigma2):
                D = (
                    Lambda + sigma2
                ) ** -1  # diag(D) = diag[ 1 / (lambda_k + sigma^2) ]
            ...
            sol = numpy.linalg.solve(
                RTDR, RTDy
            )  # inferred mean PM of the cluster
            sigval = sigma2 ** 0.5  # inferred internal PM dispersion
            z = y - numpy.dot(
                R, sol
            )  # difference between model prediction and actual PM
            H = numpy.zeros((3, 3))  # overall hessian
            H[
                :2, :2
            ] = RTDR  # top-left 2x2 block of the hessian is [ R^T  diag(D)  R ]
            H[2, 2] = numpy.sum(
                4 * sigma2 * z ** 2 * D ** 3
                - (2 * sigma2 + z ** 2) * D ** 2
                + D
            ...
            H[2, :2] = (
                numpy.dot(R.T, z * D ** 2) * 2 * sigval
            )  # two remaining values in the bottom row
            H[:2, 2] = H[
                2, :2
            ]  # ..and symmetrically transposed ones in the right column
            invH = numpy.linalg.inv(H)
            covar = invH[0:2, 0:2]  # covariance matrix for meanpmra, meanpmdec
            sigerr = (
                invH[2, 2] ** 0.5
            )  # standard deviation of sigma (don't output its correlations with meanpmra, meanpmdec)
    ...
    return (
        ...
        covar[0, 0] ** 0.5,
        covar[1, 1] ** 0.5,
        covar[0, 1] / (covar[0, 0] * covar[1, 1]) ** 0.5,
        sigval,
        sigerr,
    )
...
def create_cluster(nstar, radius, sigma=0, covfnc=None):
    ...
    sigvar = (
        sigma / (1 + (ra ** 2 + dec ** 2) / rsigma ** 2) ** 0.25
    ...
    pm_err_mag = numpy.random.gamma(
        shape=2, scale=0.5, size=nstar
    )  # size of error ellipse
    pm_err_rat = numpy.random.uniform(
        0.2, 1, size=nstar
    )  # axis ratio of --"--
    pm_err_dir = numpy.random.uniform(
        0, numpy.pi, size=nstar
    )  # orientation of --"--
    ...
    pmra_error = (
        (pm_err_maj ** 2 + pm_err_min ** 2 * tan ** 2) / (1 + tan ** 2)
    ) ** 0.5
    pmdec_error = (
        (pm_err_min ** 2 + pm_err_maj ** 2 * tan ** 2) / (1 + tan ** 2)
    ) ** 0.5
    pm_corr = (
        tan
        / (1 + tan ** 2)
        * (pm_err_maj ** 2 - pm_err_min ** 2)
    ...
    if not covfnc is None:
        ...
        chol = numpy.linalg.cholesky(covmat2)
        errs = numpy.dot(
            chol, numpy.random.normal(size=2 * nstar)
        ...
        pmdec += pmdec_error * (
            noise2 * (1 - pm_corr ** 2) ** 0.5 + noise1 * pm_corr
        )
    ...
    def ellipse(meanx, meany, sigmax, sigmay, corr, **kw):
        ...
        if corr == 0:
            return matplotlib.patches.Ellipse(
                (meanx, meany), width=2 * sigmax, height=2 * sigmay, **kw
            )
        ...
        return matplotlib.patches.Ellipse(
            (meanx, meany),
            width=(2 * (sum + det)) ** 0.5,
            height=(2 * (sum - det)) ** 0.5,
            angle=180 / numpy.pi * ang,
            **kw
        )

    def test(nstar, radius, sigma=0, covfnc=None, plot=False, fix_sigma=False):
        ra, dec, pmra, pmdec, pmra_err, pmdec_err, pm_corr = create_cluster(
            nstar, radius, sigma, covfnc
        )
        (
            meanpmra,
            meanpmdec,
            ...
            pmdec_err,
            pm_corr,
            sigma if fix_sigma else None,
            radius,
            covfnc,
        )
        print(
            "pmra=%.3f  pmdec=%.3f  pmra_err=%.3f  pmdec_err=%.3f  corr=%.3f  sigma=%.3f  sigma_err=%.3f"
