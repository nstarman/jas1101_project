...
from utils import LogNorm

class GMM_bins:
    def __init__(self, gc, bins):
        self.gc = gc
        self.bins = bins
        self.gmms = []
        self.run = False
    ...
    def __repr__(self):
        if self.run:
            if self.n_comp is None:
                return (f"Class {self.__class__.__name__}: " f" Max Component Number = {self.max_n_comp}")
            if self.n_comp is not None: 
                return (f"Class {self.__class__.__name__}: " f" Fixed Component Number = {self.n_comp}")
        else:
            return f"{self.__class__.__name__}"
        
    def run_GMM_bins(self, n_comp=None, max_n_comp=6, plot=True, verbose=False):
        gc = self.gc
        bins = self.bins
        ...
        for i in range(len(bins)-1):
            gc_b = gc[(bins[i]<gc.r)&(gc.r<bins[i+1])]
            gmm = GMM(gc_b.pm)
            gmm.run_GMM(n_comp=n_comp, max_n_comp=max_n_comp, plot=plot, verbose=verbose)
            self.gmms.append(gmm)
            
        print("Finished")
        self.run = True
            
    def plot_pred_main_pop(self, k_std=2., p_thre=0.8):
    ...
    def plot_pred_main_pop_all(self, z_bins, k_std=2., p_thre=0.8):
        if not self.run:
            return None
        bins = self.bins
...
class GMM:
    ...
    def run_GMM(self, n_comp=None, max_n_comp=6, verbose=True, plot=True, **kwargs):
    ...
    def predict_main_pop(self, data, k_std=2.):
        """ 
